...
def embed_query(query: str) -> list[float]:
    ...
    if norm in _query_embedding_cache:
        return _query_embedding_cache[norm]
        
    if _model_type == "gemini":
        vec = _active_model.embed_query(query)
    else:
        vec = _active_model.encode([query])[0].tolist()
        
    _query_embedding_cache[norm] = vec
    return vec
